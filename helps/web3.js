...
export async function getDepositAllowance(
  account,
  tokenStake,
  tokenReward,
  library
...
export async function getStaked(account, tokenStake, tokenReward, library) {
  try {
    const {contract, details} = getStakeContract(
      tokenStake,
      tokenReward,
      library,
      account
...
export async function getEarned(account, tokenStake, tokenReward, library) {
  try {
    ...
    if (details.legacy_stake) return await contract._currentRewards(account)
...
export async function getStakable(account, tokenStake, tokenReward, library) {
  try {
    ...
    if (details.legacy_stake) return await contract._maxStakeAmount()
...
export async function getIsTimeLocked(
  account,
  tokenStake,
  tokenReward,
  library
  ...
  try {
    ...
    if (!details.time_lock) return false
...
export async function getTimeLockDate(
  account,
  tokenStake,
  tokenReward,
  library
...
export async function getTimeLockUntil(
  account,
  tokenStake,
  tokenReward,
  library
...
export function getStakeContract(tokenStake, tokenReward, library, account) {
  try {
    if (
      !tokenStake ||
      !tokenReward ||
      !isAddress(tokenStake.address) ||
      tokenStake.address === AddressZero
    ) {
      throw Error(`Invalid 'token' parameter '${tokenStake.address}'.`)
    }
    const stakeContractDetails = getStakingContractDetails(
      tokenStake,
      tokenReward
    )
